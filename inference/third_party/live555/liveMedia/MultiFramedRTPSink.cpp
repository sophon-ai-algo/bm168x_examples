...
void MultiFramedRTPSink::setPacketSizes(unsigned preferredPacketSize,
  ...
  //printf("preferredPacketSize=%d,maxPacketSize=%d.\n",preferredPacketSize,maxPacketSize);
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  printf("%s,%d,rtpPayloadFormatName=%s,numChannels=%d.\n",__FUNCTION__,__LINE__,rtpPayloadFormatName, numChannels);
...
void MultiFramedRTPSink::packFrame() {
  ...
  //printf("fCurFrameSpecificHeaderSize=%d.\n",fCurFrameSpecificHeaderSize);
  ...
  if (fOutBuf->haveOverflowData()) {
    ...
    //printf("cur buf size frameSize=%d.\n",frameSize);
...
//    printf("%s,%d. call afterGettingFrame or ourHandleClosure.\n",__FUNCTION__,__LINE__);
...
::afterGettingFrame(void* clientData, unsigned numBytesRead,
  ...
  //printf("%s,%d.numBytesRead=%d,numTruncatedBytes=%d,durationInMicroseconds=%d.\n",__FUNCTION__,__LINE__,numBytesRead,numTruncatedBytes,durationInMicroseconds);
...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  if (fIsFirstPacket) {
    ...
    //printf("fIsFirstPacket is true ,get fNextSendTime.\n");
  }
  ...
  if (fInitialPresentationTime.tv_sec == 0 && fInitialPresentationTime.tv_usec == 0) {
    ...
    //printf("set fInitialPresentationTime.\n");
  }    
  ...
  if (numFrameBytesToUse == 0 && frameSize > 0) {
    ...
    //printf("%s,%d.numFrameBytesToUse=%d,frameSize=%d.\n",__FUNCTION__,__LINE__,numFrameBytesToUse,frameSize);
