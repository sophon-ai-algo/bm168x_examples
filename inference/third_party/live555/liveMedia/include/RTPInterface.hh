...
public:
  Boolean PushDataBuf(unsigned char* dataBuf, int length);
  unsigned char* DataBuf();
  int DataLength();
  Boolean IsEmpty() const;
  void Reset();
  int Space() const;
  void OffsetReadSize(int offset);
...
public:
  ...
  int fMaxSize;
  unsigned char fDataBuf[2000000*2];
  int fWriteCurPos;
  int fReadCurPos;
  unsigned int lastSendNalTick;
};
...
public:
  ...
  map<int,unsigned int> mapSockSendTick;
private:
  ...
  Boolean sendRTPorRTCPPacketOverTCP(unsigned char* data, unsigned size, int socketNum, int& sendNum);
  Boolean sendDataOverTCP(int socketNum, u_int8_t const* data, unsigned dataSize, Boolean forceSendToSucceed);
  Boolean sendDataOverTCP(int socketNum, u_int8_t const* data, unsigned dataSize, Boolean forceSendToSucceed, int& sendSize);
  void sendDataOverTCPWriteFile(int socketNum, u_int8_t const* data, unsigned dataSize);
