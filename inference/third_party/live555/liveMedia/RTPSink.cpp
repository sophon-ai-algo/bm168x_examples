...
Boolean RTPSink::lookupByName(UsageEnvironment& env, char const* sinkName,
				RTPSink*& resultSink) {
  printf("%s,%d. sinkName[%s].\n",__FUNCTION__,__LINE__,sinkName);
...
RTPSink::RTPSink(UsageEnvironment& env,
  ...
  printf("%s,%d. rtpPayloadFormatName[%s].\n",__FUNCTION__,__LINE__,rtpPayloadFormatName);
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv) {
  ...
  printf("fTimestampBase: %ld, tv: %lu.%06ld\n\t=> RTP timestamp: %ld\n",
