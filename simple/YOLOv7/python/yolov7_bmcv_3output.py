...
class YOLOV7_Detector(object):
    ...
    def inference_center_np(self, frame, use_np_file_as_input=False, input_cvimg=None):
        ...
        if not isinstance(frame, type(None)):
            ...
            result_bmimage = frame
            result_cvimage = input_cvimg
            for det in output[0]:
                ...
                result_bmimage, result_cvimage = self.drawPred(frame, int(det[5]), det[4], round(
                    left), round(top), round(right), round(bottom), input_cvimg)

            return result_bmimage, result_cvimage
    ...
    def inference_center(self, frame, use_np_file_as_input=False):
        ...
        if not isinstance(frame, type(None)):
            ...
            for i in indices:
                ...
                result_image, _ = self.drawPred(frame, classIds[i], confidences[i], round(
                    left), round(top), round(right), round(bottom))

            return result_image

    def drawPred(self, frame, classId, conf, left, top, right, bottom, input_cvimg=None):
        ...
        # bmcv暂不支持putText，暂时采用openCV进行
        if input_cvimg is not None:
            cv2.rectangle(input_cvimg, (left, top), (right, bottom), color, thickness=4)
            label = '%.2f' % conf
            label = '%s:%s' % (self.classes[classId], label)
            # Display the label at the top of the bounding box
            labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
            top = max(top, labelSize[1])
            cv2.putText(input_cvimg, label, (left, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, color, thickness=2)
            
        return frame, input_cvimg


if __name__ == '__main__':

    parser = argparse.ArgumentParser(
        description="Demo of YOLOv7 with preprocess by BMCV")

    parser.add_argument('--bmodel',
                        type=str,
                        default="../data/models/BM1684/yolov7_640_v0.1_3output_fp32_1b.bmodel",
                        required=False,
                        help='bmodel file path.')

    parser.add_argument('--labels',
                        type=str,
                        default="../data/images/coco.names",
                        required=False,
    ...
    yolov7 = YOLOV7_Detector(bmodel_path=opt.bmodel,
                             tpu_id=opt.tpu_id,
    ...
    if frame is not None:  # is picture file
        ...
        ret = decoder.read(yolov7.handle, input_bmimg)
        if ret:
            print("decode error\n")
            exit(-1)
        
        
        input_cvimg = frame # 辅助显示展现结果
        
        result_bmimage, result_cvimage = yolov7.inference_center_np(input_bmimg, opt.use_np_file_as_input, input_cvimg)
        
        save_basename, _ = os.path.splitext(os.path.basename(opt.bmodel)) 
        input_name, _ = os.path.splitext(os.path.basename(opt.input)) 
        save_basename = save_basename + '_bmcv_python_result_{}.jpg'.format(input_name)
        save_name = os.path.join(save_path, save_basename)

        yolov7.bmcv.imwrite(save_name, result_bmimage)
        if result_cvimage is not None:
            save_basename, _ = os.path.splitext(os.path.basename(opt.bmodel)) 
            input_name, _ = os.path.splitext(os.path.basename(opt.input)) 
            save_basename = save_basename + '_bmcv_python_result_{}_cv.jpg'.format(input_name)
            save_name = os.path.join(save_path, save_basename)
            cv2.imwrite(save_name, result_cvimage)
        ...
        if decoder.is_opened():
            ...
            while True:
                ...
                result_image = yolov7.inference_center(input_bmimg, use_np_file_as_input=False)
                ...
                yolov7.bmcv.imwrite(save_name, result_image)
