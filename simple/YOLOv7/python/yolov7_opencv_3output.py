...
class YOLOV7_Detector(object):
    def __init__(self, bmodel_path, tpu_id, class_names_path, confThreshold=0.5, nmsThreshold=0.5, objThreshold=0.1):
        ...
        self.confThreshold = confThreshold  # 类别的置信度阈值
        self.nmsThreshold = nmsThreshold    # IOU阈值
        self.objThreshold = objThreshold    # 是目标的置信度阈值，原版需要跟confThreshold一致
        self.ratio = 1
    ...
    def predict_center(self, input_data, use_np_file_as_input):
        ...
        if use_np_file_as_input:
            print("use numpy data as input")
            input_data = np.load("../data/input_img.npy")
        # else:
        #     print("use decode data as input")
    ...
    def do_once_proc(self, file_path):
        ...
        #np.save('input', input_data)
        cul_time = time.time()
        dets = self.predict_center(input_data, False)
        ...
        for det in output[0]:            
            ...
            box = det[:4]
            # scale to the origin image            
            x1 = int((box[0] - tx1) / ratio)
            y1 = int((box[1] - ty1) / ratio)
            x2 = int((box[2] - tx1) / ratio)
            y2 = int((box[3] - ty1) / ratio)
            
            assert x1 <= x2 or y1 <= y2, print("wrong point x1{} > x2{} or y1{} > y2{} ".format(x1, x2, y1, y2))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Demo of YOLOv7 with preprocess by OpenCV")
    ...
    if frame is not None:  # is picture file
        ...
        #result_image = yolov7.inference(frame, opt.use_np_file_as_input)
        result_image = yolov7.inference_center_np(frame, opt.use_np_file_as_input)
        print(result_image.shape)
        
        save_basename, _ = os.path.splitext(os.path.basename(opt.bmodel)) 
        input_name, _ = os.path.splitext(os.path.basename(opt.input)) 
        save_basename = save_basename + '_opencv_python_result_{}.jpg'.format(input_name)
        ...
        while ret:
            ...
            result_image = yolov7.inference(
                frame, use_np_file_as_input=False)

            print(result_image.shape)
            save_basename, _ = os.path.splitext(os.path.basename(opt.bmodel)) 
            input_name, _ = os.path.splitext(os.path.basename(opt.input)) 
            save_basename = save_basename + '_opencv_python_result_{}.jpg'.format(id)
            save_name = os.path.join(save_path, save_basename)
