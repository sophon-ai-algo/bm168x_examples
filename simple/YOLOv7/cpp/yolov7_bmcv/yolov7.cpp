...
int YoloV7::Init(float confThresh, float objThresh, float nmsThresh, const std::string& coco_names_file)
{
...
int YoloV7::pre_process(const std::vector<cv::Mat>& images, int image_n)
{
  ...
  for(int i = 0; i < image_n; ++i) {
    ...
    printf("src w=%d h=%d dst w=%d h=%d ratio=%f\n", images[i].cols, images[i].rows, m_net_w, m_net_h, ratio);
    ...
    if (isAlignWidth) {
      padding_attr.dst_crop_h = int(images[i].rows*ratio);
      padding_attr.dst_crop_w = m_net_w;
      padding_attr.dst_crop_sty = int((m_net_h - padding_attr.dst_crop_h)/2);
    }else{
      padding_attr.dst_crop_h = m_net_h;
      padding_attr.dst_crop_w = int(images[i].cols*ratio);
      padding_attr.dst_crop_stx = int((m_net_w - padding_attr.dst_crop_w)/2);
    }

    printf("dst_crop_stx=%d dst_crop_sty=%d dst_crop_w=%d dst_crop_h=%d\n", 
...
int YoloV7::Detect(const std::vector<cv::Mat>& input_images, std::vector<YoloV7BoxVec>& boxes)
{
  int ret = 0;
...
int YoloV7::post_process(const std::vector<cv::Mat> &images, std::vector<YoloV7BoxVec>& detected_boxes, int input_batch)
{
  ...
  for(int batch_idx = 0; batch_idx < input_batch; ++ batch_idx)
  {
    ...
    if (isAlignWidth) {
      offset_y =  ((float)m_net_h - (float)frame_height*ratio)/2;
    }else{
      offset_x = (m_net_w - frame_width*ratio)/2;
    }
    printf("tx1=%d ty1=%d \n", offset_x, offset_y);
    ...
    for (int i = 0; i < box_num; i++) {
      ...
      if (score > m_objThreshold)
      {
        ...
        if (confidence >= m_confThreshold)
        {
          ...
          float width = ptr[2];
          float height = ptr[3];
          float x1 = centerX - width / 2 - offset_x;
          float y1 = centerY - height / 2 - offset_y;
          ...
          printf("result: box[%d %d %d %d] class_id=%d score=%f tx1=%d ty1=%d\n", 
            box.x, box.y, box.width, box.height, box.class_id, box.score, offset_x, offset_y);
        }
      }
    }
    LOG_TS(m_ts, "post 2: filter boxes");
